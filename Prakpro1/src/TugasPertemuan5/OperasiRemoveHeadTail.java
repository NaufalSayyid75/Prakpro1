...
public class OperasiRemoveHeadTail {
	...
	while (current.next != null) { previous = current;
	current = current.next;
	}

	// Hapus elemen terakhir dispose(current);
	previous.next = null;
	...
	// Menambahkan elemen ke list dan menampilkannya myList.addHead(1);
	myList.addHead(5); 
	myList.addHead(3); 
	myList.addHead(6); 
	myList.addHead(2);
	 
	System.out.println("List before removal:"); myList.displayList();

	// Menghapus elemen pertama & terakhir dan menampilkannya kembali myList.removeHead();
	myList.removeTail();


	System.out.println("List after removal:"); myList.displayList();
	}
